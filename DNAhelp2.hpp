...
int max2Darr(int **arr, int n, int m, int &newN, int &newM)
{
    int max = 0;
    for (int j = 1; j < m + 1; j++)
    {
...
int **manhattanProblem(int n, int m, int **down, int **right, int **diag, std::string v, std::string w)
{
    ...
    for (int j = 1; j < m + 1; j++)
    {
        for (int i = 1; i < n + 1; i++)
        {
            ...
            if (j == m && i == n)
            {
                s[i][j] = maxOf2(maxOf3(s[i - 1][j] + down[i - 1][j], s[i][j - 1] + right[i][j - 1], s[i - 1][j - 1] + getMistmatchCost(v[i - 1], w[j - 1])), max2Darr(s, n, m, newN, newM));
            }

            else
            {
                s[i][j] = maxOf2(maxOf3(s[i - 1][j] + down[i - 1][j], s[i][j - 1] + right[i][j - 1], s[i - 1][j - 1] + getMistmatchCost(v[i - 1], w[j - 1])), 0);
    ...
    outputLCS(backtrack, v, newN, newM);
    std::cout << std::endl;
    outputLCS2(backtrack, w, newN, newM);
